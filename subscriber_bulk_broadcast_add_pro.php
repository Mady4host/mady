...
<div class="row multi_layout">  
  ...
  <div class="col-12 col-md-8 col-lg-8 collef">
    <form action="#" enctype="multipart/form-data" id="messenger_bot_form" method="post">
      ...
      <input type="hidden" id="selected_page_ids" name="selected_page_ids" value="">
          ...
          <div class="row">
            ...
            <div class="col-12 col-md-6">
              <div class="form-group">                
                ...
                <input type="button" class="form-control" value="select page" style="cursor:pointer" id="selectPage">
              </div>
            </div>
          </div>
  ...
  <div class="col-12 col-md-4 col-lg-4 colmid" id="middle_column">
      <div class="card main_card">
        ...
        <div class="card-body">
          <?php include(FCPATH."application/modules/messenger_bot_enhancers/views/messenger_broadcaster/summary_pro.php") ?>            
        </div>
     </div>
  </div>
</div>
...
<?php for($template_type=1;$template_type<=1;$template_type++){ ?>
  ...
  $.ajax({
    type:'POST' ,
    url: base_url+"messenger_bot_enhancers/subscriber_bulk_broadcast_add_action_pro",
    data: queryString,
    dataType : 'JSON',
    // async: false,
    cache: false,
    ...
    $(document).on('click', '#selectPage', function(){
        $("#select_pages_modal").modal()
    });
    
    $(document).on('click', '#saveSelection', function(){
        $("#select_pages_modal .close").click()
        var selectedPageIds = $('#selected_page_ids').val();
    });
  ...
  function saveSelection() {
    ...
    // Do something with the selectedPageIds, for example, send them to the server
    console.log('Selected Page IDs:', selectedPageIds);
  }
  
  function updateSelectedPageIds() {
    var selectedPageIds = $('.page-checkbox:checked').map(function () {
      return this.value;
    }).get();

    $('#selected_page_ids').val(selectedPageIds.join(','));
  }
...
<div class="modal fade" id="select_pages_modal" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="fas fa-info-circle"></i> Select Pages</h5>
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
      </div>
      <div class="modal-body">
          
        
        <!-- Display pages in a Bootstrap card for improved styling -->
        <div class="card">
            <div class="card-body">
            <div class="form-check">
              <!-- Separate "Select All" from individual pages -->
              <input class="form-check-input fffff" type="checkbox" id="select_all">
              <label class="form-check-label" for="select_all">Select All</label>
            </div>
          </div>
            <?php
              // Loop through your pages and create a checkbox for each
              ...
              $pages = $this->basic->get_data($table_name, $where, '', '', '', '', 'page_name asc');
              foreach ($pages as $page) {
                echo '<div class="card-body">';
                echo '<div class="form-check">';
                echo '<input class="form-check-input page-checkbox" type="checkbox" value="' . $page['id'] . '" id="page_' . $page['id'] . '">';
                echo '<label class="form-check-label" for="page_' . $page['id'] . '">' . $page['page_name'] . '</label>';
                echo '</div>';
                echo '</div>';
              }
            ?>
