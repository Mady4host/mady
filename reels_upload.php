...
.upload-body { padding: 32px 24px 24px 24px; }
...
.upload-section-title .icon { font-size: 1.15em; color:#2196f3; }
...
@media (max-width: 767.98px) { #pageSearch { width:100%; } }
.hashtag-btn{background:linear-gradient(90deg,#f0f6ff 70%,#eaf3fa 100%);border:1px solid #b6d1fa;border-radius:16px;padding:3px 13px;font-size:13.5px;cursor:pointer;margin:2px;color:#0d4e96;font-weight:600;}
...
@media (max-width: 767.98px) { #videoDrop{padding:18px;} }
...
@media (max-width: 767.98px) { .video-card { flex-direction:column; gap: 14px; padding:12px; } }
...
.remove-video{position:absolute;top:10px;left:10px;font-size:13px;background:#dc3545;color:#fff;border:none;border-radius:20px;padding:6px 15px;cursor:pointer;box-shadow:0 2px 10px rgba(220,53,69,.07);}
...
<div class="upload-main-card">
  ...
  <div class="upload-body">
    ...
    <!-- تم إيقاف رسائل الفلاش القديمة حتى لا تظهر بعد إعادة التحميل -->
    <!--
    <?php if(false && $this->session->flashdata('msg_success')): ?>
        <div class="alert alert-success auto-hide"><?= $this->session->flashdata('msg_success') ?></div>
    <?php endif; ?>
    <?php if(false && $this->session->flashdata('msg')): ?>
...
<script>
/* Helpers for SweetAlert */
function escapeHtml(s){return String(s).replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
function swalSuccess(title, lines, redirectUrl){
  const html = (lines && lines.length)
    ? '<div style="text-align:right;line-height:1.7">'+lines.map(l=>'<div style="margin-bottom:6px">'+l+'</div>').join('')+'</div>'
    : '';
  Swal.fire({
    icon:'success',
    ...
    allowEscapeKey:true,
    backdrop:true
  }).then(()=>{ window.location.href = redirectUrl || "<?= site_url('reels/list') ?>"; });
}
function swalError(msg){
  Swal.fire({icon:'error',title:'حدث خطأ',text:msg||'تعذر إكمال العملية',confirmButtonText:'حسناً'});
}
...
function handleFiles(list){
    ...
    Array.from(list).forEach(f=>{
        ...
        if(f.size > MAX_MB*1024*1024){ swalError('حجم الفيديو أكبر من '+MAX_MB+'MB: '+(f.name||'')); return; }
        addVideoCard(f);
    });
}
...
function addVideoCard(file){
    ...
    remove.onclick=()=>{ 
        try{ if(vid && vid.src && vid.src.startsWith('blob:')) URL.revokeObjectURL(vid.src); }catch(e){}
        try{
          if(videoItems[idx] && Array.isArray(videoItems[idx].frames)){
            videoItems[idx].frames.forEach(fr=>{ try{ if(fr.url && fr.url.startsWith('blob:')) URL.revokeObjectURL(fr.url); }catch(_){} });
          }
        }catch(_){}
        videoItems[idx]=null; card.remove(); 
    };
    ...
    descCounter.style.fontSize='11px';
    descCounter.style.textAlign='left';
    descCounter.style.direction='ltr';
    const updateDc=()=>{ descCounter.textContent = (desc.value.length)+' chars'; };
    desc.addEventListener('input',updateDc); updateDc();
...
(function(){
  ...
  window.buildStoryPhotoFormData = function(form){
    ...
    previews.forEach((wrapper)=>{
      ...
      let match = originalFiles.find(f => f.name === fname && f.size == fsize) || originalFiles.find(f => f.name === fname);
      if(!match) return;
...
document.getElementById('reelsForm').addEventListener('submit',function(e){
    ...
    if(!this.querySelector('input[name="fb_page_ids[]"]:checked')){
        return swalError('اختر صفحة واحدة على الأقل');
    }
    if(mediaType==='story_photo'){
        const storyInput = document.getElementById('story_photo_input');
        const gallery = document.getElementById('story_preview_gallery');
        if(!storyInput || !storyInput.files || storyInput.files.length===0){
            return swalError('اختر صورة للستوري');
        }
        if(gallery.children.length === 0){
            return swalError('لا توجد صور محددة (أزلت كل المعاينات)');
        }
    ...
    if(mediaType==='story_video' || mediaType==='reel'){
        if(videoItems.filter(v=>v!==null).length===0){
            return swalError('أضف فيديو واحد على الأقل');
        }
        const fd=new FormData();
...
function uploadWithProgress(fd,actionUrl){
    ...
    xhr.onreadystatechange=()=>{
        if(xhr.readyState===4){
            // حضّر تفاصيل العرض للصندوق
            const checkedPages = Array.from(document.querySelectorAll('input[name="fb_page_ids[]"]:checked')).map(cb=>{
              const name = cb.closest('.page-row')?.querySelector('span')?.textContent?.trim() || '';
              return 'الصفحة: '+escapeHtml(name)+' ('+escapeHtml(cb.value)+')';
            });
            const isStoryPhoto = (mediaTypeInput.value==='story_photo');
            let details = [];
            if(isStoryPhoto){
              const gallery = document.getElementById('story_preview_gallery');
              const items = Array.from(gallery.children);
              const files = items.map((w,i)=> 'صورة: '+escapeHtml(w.dataset.filename || ('#'+(i+1))));
              details = [...checkedPages, ...files];
            } else {
              const vids = videoItems.filter(Boolean).map(v=> 'فيديو: '+escapeHtml(v.file.name));
              details = [...checkedPages, ...vids];
            }

            if(xhr.status===200){
                bar.style.width='100%'; bar.textContent='تم';
                // حاول قراءة JSON للريديركت إن وُجد
                let redirectUrl = "<?= site_url('reels/list') ?>";
                try{
                  const j = JSON.parse(xhr.responseText||'{}');
                  if(j && j.redirect_url) redirectUrl = j.redirect_url;
                }catch(_){}
                const title = isStoryPhoto ? 'تم رفع/جدولة القصص بنجاح' : 'تم رفع/جدولة الريلز/الستوري بنجاح';
                try{ window.parent && window.parent.postMessage({type:'upload:done', source:'fb'}, '*'); }catch(_){}
                swalSuccess(title, details, redirectUrl);
            } else {
                try {
                    const json = JSON.parse(xhr.responseText||'{}');
                    swalError(json.msg || ('خطأ أثناء الإرسال (HTTP '+xhr.status+')'));
                    try{ window.parent && window.parent.postMessage({type:'upload:error', source:'fb', message:json.msg||('HTTP '+xhr.status)}, '*'); }catch(_){}
                } catch(e){
                    swalError('خطأ أثناء الإرسال (HTTP '+xhr.status+')');
                    try{ window.parent && window.parent.postMessage({type:'upload:error', source:'fb', message:'HTTP '+xhr.status}, '*'); }catch(_){}
                }
            }
        }
    };
...
/* إخفاء أي رسائل قديمة لو موجودة (لن تُعرض أصلاً) */
setTimeout(()=>{
