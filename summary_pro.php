...
<script type="text/javascript">
	...
	$("document").ready(function(){	
		...
		$(document).on('change','#page,#label_ids,#excluded_label_ids,#user_gender,#user_time_zone,#user_locale,#broadcast_type,.page-checkbox, .fffff',function(){     
		  var page_id=$("#selected_page_ids").val();
		  ...
		  $.ajax({
			  type:'POST' ,
			  url: base_url+"home/get_broadcast_summary_pro",
			  ...
			  success:function(response){
			      ...
			      var 
			        t_estimated_reach = 0,
			        t_total_subscriber_count = 0,
			        t_subscriber_count = 0,
			        data = ''; 
			        
			      response.pageinfo.forEach(function(page) {
			          let estimated_reach = parseInt(page.estimated_reach);
		              if(estimated_reach=="") estimated_reach='0';
		              
		              let subscriber_count = parseInt(page.subscriber_count);
		              if(subscriber_count=="") subscriber_count='0';
		              
		              let total_subscriber_count = parseInt(page.total_subscriber_count);
		              if(total_subscriber_count=="") total_subscriber_count='0';
		              
			          t_estimated_reach += estimated_reach;
			          t_total_subscriber_count += total_subscriber_count;
			          t_subscriber_count += subscriber_count;
			          if(is_quick_broadcast=='1'){
			              data += `<li class="list-group-item d-flex justify-content-between align-items-center">
                                	${page.page_name} <?php echo $this->lang->line("Subscribers"); ?> 
                                	<span class="badge badge-status" >${estimated_reach}</span>
                              	</li>
                        	    <li class="list-group-item d-flex justify-content-between align-items-center active">
                        	        ${page.page_name} <?php echo $this->lang->line("Reach"); ?>
                        	        <span class="badge badge-status">${subscriber_count}</span>
                        	    </li>`;
			          }else{
			              data += `<li class="list-group-item d-flex justify-content-between align-items-center">
                                	${page.page_name} <?php echo $this->lang->line("Subscribers"); ?> 
                                	<span class="badge badge-status" >${total_subscriber_count}</span>
                              	</li>
                        	    <li class="list-group-item d-flex justify-content-between align-items-center active">
                        	        ${page.page_name} <?php echo $this->lang->line("Reach"); ?>
                        	        <span class="badge badge-status">${subscriber_count}</span>
			    ...
			    if(is_quick_broadcast=='1') $("#page_subscriber").html(t_estimated_reach);
			    else $("#page_subscriber").html(t_total_subscriber_count);

          		$("#targetted_subscriber").html(t_subscriber_count);
