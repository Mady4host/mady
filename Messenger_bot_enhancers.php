...
In this case, we suggest you to check the error message in report, and if you think it’s not for your message content, but for specific subscribers, you can restart the campaign from where it is left off, by clicking on the option menu & then click Force Resume. ").'"><i class="fas fa-info-circle"></i> </a>';
    ...
    public function subscriber_delete_all_campaign()
    {   
        $this->ajax_check();
        $this->csrf_token_check();
        ...
        $campaigns = $this->basic->get_data("messenger_bot_broadcast_serial", array("where" => array("user_id" => $this->user_id)));
    
        foreach ($campaigns as $campaign) {
            $id = $campaign['id'];
            $posting_status = $campaign['posting_status'];
    
            if ($this->basic->delete_data("messenger_bot_broadcast_serial", array("id" => $id, "user_id" => $this->user_id))) {
                $this->basic->delete_data("messenger_bot_broadcast_serial_send", array("campaign_id" => $id, "user_id" => $this->user_id));
                
                // Remove usage data if the campaign is pending
                if ($posting_status != "2") {
                    $this->_delete_usage_log($module_id = 211, $request = $campaign["total_thread"]);
                }
            }
        }
    
        // Success response
        echo "All eligible campaigns have been deleted.";
    }
    
    public function force_reprocess_all_campaign()
    {
        $this->ajax_check();
        $this->csrf_token_check();
        ...
        $where = array('posting_status' => '4', 'user_id' => $this->user_id);
        ...
        if($this->db->affected_rows() > 0) {
            echo "all on-hold campaings have refreshed";
        } else {
            echo "no on-hold campaings found"; // Indicating no changes were made
        }
    }
    
    public function subscriber_delete_all_compeleted_on_hold_campaign()
    {   
        $this->ajax_check();
        $this->csrf_token_check();
        ...
        foreach ($campaigns as $campaign) {
            ...
            if ($posting_status == '2' || $posting_status == '4') {
                // Delete the campaign
                if ($this->basic->delete_data("messenger_bot_broadcast_serial", array("id" => $id, "user_id" => $this->user_id))) {
                    $this->basic->delete_data("messenger_bot_broadcast_serial_send", array("campaign_id" => $id, "user_id" => $this->user_id));
        ...
        // Success response
        echo "All completed and on-hold campaigns have been deleted.";
    }
